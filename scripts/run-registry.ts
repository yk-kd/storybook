...
const currentVersion = async () => {
  const { version } = await Bun.file(path.join(__dirname, '..', 'code', 'package.json')).json();
  return version;
};
...
const publish = async (packages: { name: string; location: string }[], url: string) => {
   ...
   * such as "workspace:*"
   *
   * We can't publish to npm if the package.json contains these ranges. So with `npm pack` we create a tarball that we can publish.
   *
